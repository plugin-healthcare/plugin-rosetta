...
- [x] GIVEN the retained `sssom-rosetta` workflows, WHEN the parity comparison runs, THEN every retained workflow is listed with its `plugin-rosetta` replacement command and its replacement test name.
...
- [x] GIVEN any engineering difference found in the comparison, WHEN the release is assessed, THEN it is removed or recorded as a dated, justified entry pending final curator approval.
...
- [x] GIVEN the CLI, WHEN the command surface is reviewed, THEN every command delegates to an application function and no command body contains parsing, validation, graph, or serialisation logic.
- [x] GIVEN a clean environment with no project checkout on the path, WHEN the built wheel is installed, THEN `rosetta --help` runs and the example completes successfully.
- [x] GIVEN the README quickstart, WHEN a new user follows it end to end, THEN they install the package, validate the preserved mapping set, build its artifacts, and read the report.
- [x] GIVEN the runnable example, WHEN it runs on a clean checkout without any licensed release, THEN it completes using only tracked content and produces open-format outputs.
- [x] GIVEN the capability matrix, WHEN it is read, THEN it lists every migrated source, every published format, every explicitly deferred behaviour, and every non-migrated legacy module with its reason.
- [x] GIVEN every published output format, WHEN it is read by a standard tool in a process that does not import `plugin_rosetta`, THEN it parses and yields the expected content.
- [x] GIVEN the release candidate, WHEN `tara check` runs, THEN it passes.

## Technical Tasks

- [x] Build the retained-workflow inventory from the pinned `sssom-rosetta` recipes and commands, excluding deferred Protege and Gephi workflows.
- [x] For each retained workflow, name the replacement command and replacement test.
...
- [ ] Obtain curator approval for each remaining documented difference in `docs/capability-matrix.md`.
- [x] Review every CLI command for thinness and keep behavior in the owning public feature package.
- [x] Add `examples/build_preserved_mapping_set.py` that validates, builds, and reports the tracked `omop-onz-g` mapping set using only tracked content.
- [x] Add a README quickstart covering install, validate, build, report, and where outputs land.
- [x] Build the wheel and install it in a clean environment, then run `rosetta --help` and the example from that environment.
- [x] Add `tests/test_parity.py` asserting that every retained workflow in the inventory has a replacement test, so the matrix cannot silently drift.
...
- [x] `uv run pytest` passes for the whole suite, including `tests/test_parity.py`.
- [x] `uv run tara check` passes.
- [x] `README.md`, `docs/capability-matrix.md`, and `registry/README.md` are current, and the runnable example under `examples/` works.
- [x] Every published format is proven readable by a standard tool in a process that does not import `plugin_rosetta`.
...
- [x] No licensed data, cached downloads, generated artifacts, or credentials are staged.
