...
- [x] Add `src/plugin_rosetta/vocabulary/omop.py` with `load_target_concepts`, `load_relationships`, `load_relationship_types`, and `build_graph`.
...
- [x] Add `tests/vocabulary/test_omop.py` and `tests/vocabulary/test_templates.py` driven by small in-memory Polars frames and the synthetic fixture release.
...
| `src/sssom_rosetta/vocabulary/omop.py` `build_from_release`, `_exact`, `write_ttl` | `src/plugin_rosetta/vocabulary/omop.py` plus `src/plugin_rosetta/vocabulary/_graph_io.py` | Split release location from serialisation |
...
- [x] `uv run pytest tests/vocabulary/test_omop.py tests/vocabulary/test_templates.py` passes offline against synthetic fixtures.
